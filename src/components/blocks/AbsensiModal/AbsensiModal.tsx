...
export function AbsensiModal({ open, onOpenChange, absenType = 'masuk', onAbsenSuccess }) {
    ...
    const targetLocation = { lat: -6.55989, lng: 106.79296 };
    const allowedRadius = 200;

    const { mutate: absenMasuk, isPending: isMasukPending } = useMutation({
        mutationFn: (formData: FormData) => postDosenServices.addDataAbsensiMasuk(formData),
        onSuccess: () => {
        ...
        onError: (error: any) => {
            const errorMessage = error.response?.data?.message || "Gagal absen masuk.";
            toast.error(errorMessage);
        },
    });

    const { mutate: absenKeluar, isPending: isKeluarPending } = useMutation({
        mutationFn: (formData: FormData) => postDosenServices.addDataAbsensiKeluar(formData),
        onSuccess: () => {
        ...
        onError: (error: any) => {
            const errorMessage = error.response?.data?.message || "Gagal absen keluar.";
            toast.error(errorMessage);
        },
    });
    ...
    const onSubmit = (data) => {
        ...
        if (!userLocation.lat) {
            toast.error("Lokasi tidak terdeteksi", { description: "Harap tunggu atau aktifkan izin lokasi." });
            return;
        }
    ...
    useEffect(() => {
        if (open) {
            const defaultKeterangan = absenType === 'masuk'
                ...
                <form onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-center my-4">
                        <div>
                            ...
                            <p className="font-semibold">{new Date().toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}</p>
                        </div>
                        <div>
                            <p className="text-sm text-gray-500">Lokasi</p>
                            {latitude ? (
                                <p className="font-semibold text-xs">{`${latitude.toFixed(4)}, ${longitude.toFixed(4)}`}</p>
                            ) : (
                                <p className="font-semibold text-xs text-red-500">{locationError || 'Mencari lokasi...'}</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="w-full aspect-video bg-gray-200 rounded-md overflow-hidden flex items-center justify-center">
                        {foto ? (
                            <img src={URL.createObjectURL(foto)} alt="Captured" className="w-full h-full object-cover"/>
                        ) : (
                            <Webcam
                                audio={false}
                                ref={webcamRef}
                    ...
                    <div className="flex justify-center mt-2">
                        {foto ? (
                            <Button type="button" variant="outline" onClick={() => setValue('foto', null)}>Ambil Ulang</Button>
                        ) : (
