...
// --- Skema Validasi Zod ---
const evaluasiKerjaSchema = z.object({
  pegawai_id: z.number(),
  ...
  nilai_pendidikan: z.coerce
    .number()
    .min(0, "Nilai min 0")
    .max(100, "Nilai maks 100"),
  nilai_penelitian: z.coerce
    .number()
    .min(0, "Nilai min 0")
    .max(100, "Nilai maks 100"),
  nilai_pengabdian: z.coerce
    .number()
    .min(0, "Nilai min 0")
    .max(100, "Nilai maks 100"),
...
export type EvaluasiKerjaFormValues = z.infer<typeof evaluasiKerjaSchema>;
...
const DetailEvaluasiKerja = () => {
  ...
  // --- Form ---
  const form = useForm<EvaluasiKerjaFormValues>({
    resolver: zodResolver(evaluasiKerjaSchema),
    defaultValues: {
  ...
  const { mutate: submitEvaluation, isPending: isSubmitting } = useMutation({
    mutationFn: (data: EvaluasiKerjaFormValues) =>
  ...
  const onSubmit = (values: EvaluasiKerjaFormValues) => {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
              ...
              <div className="flex flex-col gap-4">
                ...
                <FormFieldInput
                  ...
                  label="5. Penerapan Tridharma"
                  type="number"
                />
                <FormFieldInput
                  form={form}
                  name="nilai_komitmen_disiplin"
                  label="6. Komitmen & Disiplin"
                  type="number"
                />
                <FormFieldInput
                  form={form}
                  name="nilai_kepemimpinan_kerjasama"
                  label="7. Kepemimpinan & Kerjasama"
                  type="number"
                />
                <FormFieldInput
                  form={form}
                  name="nilai_inisiatif_integritas"
                  label="8. Inisiatif & Integritas"
                  type="number"
                />
              </div>
            </div>
