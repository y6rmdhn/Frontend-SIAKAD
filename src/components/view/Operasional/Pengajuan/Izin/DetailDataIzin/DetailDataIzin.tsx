...
const DetailDataIzin = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["detail-pengajuan-izi-dosen", params.id],
    queryFn: async () => {
      if (!params.id) return null;
      const response = await dosenServices.getDataIzinWithoutParams(params.id);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 border border-black rounded-lg p-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-col sm:flex-row gap-2 justify-between border-b p-2">
                  ...
                  <Label className="text-xs sm:text-sm">
                    {data?.jenis_izin ? data.jenis_izin : "-"}
                  </Label>
                </div>
                <div className="flex flex-col sm:flex-row gap-2 justify-between border-b p-2">
                  <Label className="font-semibold text-[#2572BE] text-xs sm:text-sm w-38 shrink-0">
                ...
                <div className="flex flex-col sm:flex-row gap-2 justify-between border-b p-2">
                  ...
                  <Label className="text-xs sm:text-sm">
                    {data?.data.alasan_izin ? data.data.alasan_izin : "-"}
                  </Label>
                </div>
                <div className="flex flex-col sm:flex-row gap-2 justify-between border-b p-2">
                  <Label className="font-semibold text-[#2572BE] text-xs sm:text-sm w-38 shrink-0">
