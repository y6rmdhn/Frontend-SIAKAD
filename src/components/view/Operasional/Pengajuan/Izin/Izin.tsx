...
const Izin = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const jenisIzin = searchParam.get("jenis_izin_id");
      const statusPengajuan = searchParam.get("status_pengajuan");

      if (search) params.search = search;
      if (jenisIzin && jenisIzin !== "semua") params.jenis_izin_id = jenisIzin;
      if (statusPengajuan && statusPengajuan !== "semua")
        params.status = statusPengajuan;

      const response = await dosenServices.getDataIzin(params);
  ...
  const jenisIzinOptions = useMemo(() => {
    const options = data?.filter_options?.jenis_izin?.map((item: any) => ({
      label: item.nama,
      value: item.id.toString(),
    })) || [];
    return [{ label: "Semua Jenis Izin", value: "semua" }, ...options];
  }, [data]);
  ...
  return (
    <div className="mt-10 mb-20">
      <Title title="Izin" subTitle="Daftar Pengajuan Izin" />
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div className="flex gap-4 min-[864px]:flex-row flex-col">
              ...
              <SelectFilter
                ...
                value={searchParam.get("jenis_izin_id") || "semua"}
                options={jenisIzinOptions}
                onValueChange={(value) =>
                  handleFilterChange("jenis_izin_id", value)
                }
              />
            </div>
          </div>
      ...
      <Table className="mt-6 table-auto text-xs lg:text-sm">
        <TableHeader>
          <TableRow className="bg-[#002E5A]">
            ...
            <TableHead className="text-center text-white">Jenis Izin</TableHead>
            ...
            <TableHead className="text-center text-white">Jumlah Hari</TableHead>
          ...
          {isLoading ? (
            ...
            items.map((item: any, index: number) => (
              <TableRow key={item.id} className="even:bg-gray-100">
                ...
                <TableCell className="text-center">
                  {item.jenis_izin?.nama ?? item.jenis_izin ?? "-"}
                </TableCell>
                <TableCell className="text-center">
                ...
                <TableCell>
                  <div className="flex justify-center items-center gap-2">
                    ...
                    <Button 
                      ...
                      onClick={() => {
                        const fileUrl = item.file_izin?.url || item.file_pendukung?.url || item.file_url;
                        if (fileUrl) {
