...
const MAX_FILE_SIZE_MB_DISPLAY = "2.007 MB"; // Tetap sama
const MAX_FILE_SIZE_BYTES = Math.floor(2.007152 * 1024 * 1024); // Tetap sama

// Tipe MIME yang diterima HANYA PDF sekarang
const ACCEPTED_MIME_TYPES_PDF_ONLY = [
    "application/pdf"
];

const optionalFilePdfOnlySchema = z
    .custom<FileList>((val): val is FileList => val instanceof FileList, { // Console.log bisa dihapus jika sudah tidak diperlukan
        message: "Input file tidak valid.",
    })
    .refine(
        (files) => {
            if (!files || files.length === 0) { // Menambahkan !files untuk keamanan
                return true;
            }
            return ACCEPTED_MIME_TYPES_PDF_ONLY.includes(files[0].type);
        },
        {
            message: "Jenis file tidak valid. Hanya file PDF yang diperbolehkan.", // Pesan disesuaikan
        }
    )
    .refine(
        (files) => {
        ...
        {
            message: `Ukuran file maksimal adalah ${MAX_FILE_SIZE_MB_DISPLAY}.`,
        }
    )
    .optional()
    .nullable();
...
const pangkatSchema = z.object({
    jenis_sk_id: z.string({ required_error: "Jenis SK tidak boleh kosong." }).min(1, "Jenis SK tidak boleh kosong."),
    jenis_kenaikan_pangkat_id: z.string({ required_error: "Jenis kenaikan pangkat tidak boleh kosong." }).min(1, "Jenis kenaikan pangkat tidak boleh kosong."),
    pangkat_id: z.string({ required_error: "Nama pangkat tidak boleh kosong." }).min(1, "Nama pangkat tidak boleh kosong."),

    tmt_pangkat: z.string({ required_error: "TMT. Pangkat tidak boleh kosong." })
        .min(1, "TMT. Pangkat tidak boleh kosong.")
        .regex(/^\d{4}-\d{2}-\d{2}$/, "Format TMT. Pangkat tidak valid (YYYY-MM-DD)"),
    no_sk: z.string({ required_error: "No SK tidak boleh kosong." }).min(1, "No SK tidak boleh kosong."),

    file_pangkat: optionalFilePdfOnlySchema, // Menggunakan skema file PDF saja

    tgl_sk: z.string({ required_error: "Tanggal SK tidak boleh kosong." })
        .min(1, "Tanggal SK tidak boleh kosong.")
        .regex(/^\d{4}-\d{2}-\d{2}$/, "Format Tgl. SK tidak valid (YYYY-MM-DD)"),

    pejabat_penetap: z.string({ required_error: "Pejabat penetap tidak boleh kosong." }).min(1, "Pejabat penetap tidak boleh kosong."),

    masa_kerja_tahun: z.preprocess(
        (val) => {
        ...
        z.coerce
            .number({ required_error: "Masa Kerja (Tahun) tidak boleh kosong.", invalid_type_error: "Masa Kerja (Tahun) harus berupa angka." })
            .int("Masa Kerja (Tahun) harus bilangan bulat.")
            .nonnegative("Masa Kerja (Tahun) tidak boleh negatif.")
            .max(50, "Masa Kerja (Tahun) tidak boleh lebih dari 50.")
    ),

    masa_kerja_bulan: z.preprocess(
        (val) => (val === "" || val === null || val === undefined || Number.isNaN(Number(val)) ? undefined : Number(val)), // Konversi ke angka atau undefined
        z.coerce
...
const DetailPangkat = () => {
    ...
    const { mutate, isPending: isSubmitting } = useMutation({
        mutationFn: (formData: FormData) => postDosenServices.addDataPangkat(formData),
        onSuccess: (response) => {
    ...
    const handleSubmitPangkat = (values: PangkatFormData) => {
        ...
        (Object.keys(values) as Array<keyof PangkatFormData>).forEach(key => {
            if (key !== 'file_pangkat') {
                ...
                if (key === 'masa_kerja_tahun' && (valueToAppend === null || valueToAppend === undefined)) {
                    valueToAppend = 0;
                }
                        ...
                        actions={
                            <div className=" flex justify-start md:justify-end mt-10">
                                <div className="flex w-full md:w-auto flex-col md:flex-row gap-4">
                                    ...
                                    <Button
                                        className="w-full sm:w-auto bg-[#FDA31A] hover:bg-[#329C59] cursor-pointer flex items-center gap-2 text-xs md:text-sm">
                                        {isSubmitting ? (
                                            "Menyimpan..."
                                        ) : (
                                            <>
