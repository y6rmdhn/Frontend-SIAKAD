...
import {FormFieldSelect} from "../../../../../blocks/CustomFormSelect/CustomFormSelect";

const MAX_FILE_SIZE = 5 * 1024 * 1024;
const ACCEPTED_MIME_TYPES = ["application/pdf"];

const detailJabatanStrukturalSchema = z.object({
    ...
    file_jabatan: z
        .any()
        .optional() // Tetap optional, tidak wajib upload file
        .refine(
...
const DetailJabatanStruktural = () => {
    ...
    const {data: detailData} = useQuery({
        ...
        queryFn: async () => {
            const response = await dosenServices.getDataJabatanakstrukturalWithoutParam();
    ...
    const {mutate} = useMutation({
        mutationFn: (formData: FormData) => postDosenServices.addDataJabatanstruktural(formData),
        onSuccess: (response) => {
                ...
                <form onSubmit={form.handleSubmit(handleSubmitJabatanStruktural)}>
                    ...
                    <div className="mt-10 grid md:grid-rows-5 md:grid-flow-col md:items-center gap-6 w-full">
                        <FormFieldSelect
                            ...
                            options={[
                                {value: "1", label: "Wakil Rektor II"},
                                {value: "2", label: "Wakil Rektor IV"},
                                {value: "3", label: "Direktur Pascasarjana"},
                                {value: "4", label: "Kepala Lembaga"},
                            ]}
                            required={false}
