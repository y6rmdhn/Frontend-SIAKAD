...
const JabatanAkademik = () => {
  ...
  // get data
  const { data } = useQuery({
    queryKey: ["jabatan-akademik-dosen", searchParam.get("page")],
    queryFn: async () => {
      const response = await dosenServices.getJabatanAkademik(
        ...
        <TableBody className="divide-y divide-gray-200">
          {data?.data.data.map((item: any) => (
            <TableRow className=" even:bg-gray-100">
              ...
              <TableCell className="text-center">
                {item.tgl_sk
                  ? format(parseISO(item.tgl_sk), "dd MMMM yyyy")
                  : "-"}
              </TableCell>
  ...
  `}
              ...
              <TableCell className="h-full">
                <div className="flex justify-center items-center w-full h-full">
                  <Link
                    to={
                      "/data-riwayat/kepegawaian/detail-data-jabatan-akademik/" +
                      item.id
                    }
                  >
                    <Button
