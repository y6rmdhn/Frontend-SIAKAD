...
const Pasangan = () => {
  ...
  const statusPengajuanOptions = useMemo(() => {
    return (
      data?.filters?.status_pengajuan?.map((item) => ({
        label: item.nama,
        value: item.id,
      })) || []
    );
  ...
  const { mutate: deleteData } = useMutation({
    mutationFn: (id: string) => deleteDataDosenServices.deteleDataPasangan(id),
    onSuccess: () => {
                  ...
                  <TableRow key={item.id} className=" even:bg-gray-100">
                    ...
                    <TableCell className="h-full">
                      <div className="flex justify-center items-center w-full h-full">
                        <Link
                          to={`/data-riwayat/keluarga/detail-data-pasangan/${item.id}`}
                        >
                          <Button
                            size="icon"
                            variant="ghost"
