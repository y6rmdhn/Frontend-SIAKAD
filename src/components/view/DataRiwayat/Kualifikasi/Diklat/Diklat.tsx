...
const Diklat = () => {
    ...
    // get data
    const {data} = useQuery({
        queryKey: ["diklat-dosen", searchParam.get("page")],
        queryFn: async () => {
            const response = await dosenServices.getDiklat(searchParam.get("page"));
                ...
                <TableBody className="divide-y divide-gray-200">
                    {data?.data.data.map((item) => (
                        <TableRow key={item.id} className=" even:bg-[#E7ECF2]">
                            ...
                            <TableCell className="text-center">{item.tahun_penyelenggaraan}</TableCell>
                            <TableCell className="text-center">
                                {item.timestamps.tgl_diajukan ? format(parseISO(item.timestamps.tgl_diajukan), "dd MMMM yyyy")
                                    : "-"}
                            </TableCell>
                            <TableCell className="text-center">
                                <Button
