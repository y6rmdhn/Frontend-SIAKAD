...
const dataPegawaiSchema = z.object({
  ...
  jenis_kendaraan: z
    ...
    .optional(), // max(1) sepertinya terlalu pendek
...
const DataPegawai = () => {
  ...
  const cityOptions =
    citiesData?.data?.map((city) => ({ label: city.name, value: city.code })) ||
    [];
  const kecamatanOptions =
    kecamatanData?.data?.map((kec) => ({ label: kec.name, value: kec.code })) ||
    [];
  ...
  const statusPernikahanOptions =
    statusPernikahanData?.data?.map((item) => ({
