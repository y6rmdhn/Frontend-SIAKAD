...
const formSchema = z.object({
  ...
  nama_data: z.string().min(1, "Nama data wajib diisi."),
  tanggal_data: z.string().min(1, "Tanggal wajib diisi."),
...
const TemplateTambahValidasi = () => {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: (formData: FormData) => {
      /* 
               TODO: Buat fungsi di service (misal: postAdminServices.addDataX) 
                        lalu panggil di sini.
                              */
      console.log("Submit FormData:", Object.fromEntries(formData));
      return Promise.resolve(); // Hapus ini jika sudah ada API asli
    },
    onSuccess: () => {
  ...
  const onSubmit = (values: FormSchemaType) => {
    ...
    formData.append("nama_data", values.nama_data);
    formData.append("tanggal_data", values.tanggal_data);
...
export default TemplateTambahValidasi;
