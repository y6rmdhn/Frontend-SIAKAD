...
const PenghargaanPenunjang = () => {
  ...
  const handleSuccess = (action: ActionType) => {
    ...
    queryClient.invalidateQueries({ queryKey: ["penghargaan-validasi-data"] });
  };
  ...
  if (isLoading) {
    return (
      <div className="mt-10 mb-20">
        <Title title="Validasi Penghargaan" subTitle="Daftar Pengembangan" />
      ...
      <Table className="mt-5 text-xs lg:text-sm">
        ...
        <TableBody>
          {tableData.data?.length === 0 ? (
            <TableRow>
              <TableCell
                colSpan={data?.table_columns?.length + 1 || 7}
            ...
            tableData.data.map((item: any) => (
              <TableRow key={item.id} className="even:bg-gray-50">
                ...
                <TableCell className="text-center">
                  {item.tanggal_penghargaan_formatted}
                </TableCell>
                ...
                <TableCell>
                  <div className="flex justify-center">
                    <Link
                      to={`/admin/validasi-data/penunjang/detail-data-penghargaan/${item.id}`}
                    >
