...
const Organisasi = () => {
  ...
  const jenisOrganisasiFilter = searchParam.get("jenis_organisasi") || "";
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: [
      "organisasi-validasi-data",
      currentPage,
      debouncedInput,
      unitKerjaFilter,
      jenisOrganisasiFilter,
      statusFilter,
    ],
    queryFn: async () => {
      const params = {
        ...
        jenis_organisasi: jenisOrganisasiFilter,
        status_pengajuan: statusFilter,
      };
      const response = await adminServices.getOrganisasiValidasiData(params);
  ...
  const filterOptions = useMemo(() => {
    ...
    const mapToOptions = (items: any[], valueKey = "id", labelKey = "nama") =>
      items?.map((opt: any) => ({
        value: String(opt[valueKey] ?? opt.id ?? opt.value),
        label: opt[labelKey] ?? opt.label ?? opt.nama,
      })) || [];

    return {
      unitKerja: mapToOptions(filters.unit_kerja_options, "value", "label"),
      jenisOrganisasi: mapToOptions(
        filters.jenis_organisasi_options,
        "value",
        "label"
      ),
      statusPengajuan: mapToOptions(filters.status_pengajuan, "id", "nama"),
    };
  }, [data]);
  ...
  const handleSuccess = (action: ActionType) => {
    ...
    queryClient.invalidateQueries({ queryKey: ["organisasi-validasi-data"] });
  };
  ...
  const { mutate: rejectMutation } = useMutation({
    mutationFn: (payload: { ids: number[]; keterangan: string }) =>
      patchDataServices.rejectDataRiwayatOrganisasi(payload),
  ...
  const { mutate: approveMutation } = useMutation({
    mutationFn: (payload: { ids: number[] }) =>
      patchDataServices.approveDataRiwayatOrganisasi(payload),
  ...
  if (isLoading) {
    return (
      <div className="mt-10 mb-20">
        <Title title="Validasi Organisasi" subTitle="Daftar Pengembangan" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <SelectFilter
                ...
                value={jenisOrganisasiFilter}
                onValueChange={(v) => handleUrlChange("jenis_organisasi", v)}
              />
            </div>
            <div className="flex flex-col gap-2">
      ...
      <Table className="mt-5 text-xs lg:text-sm">
        ...
        <TableBody>
          {tableData.data?.length === 0 ? (
            <TableRow>
              <TableCell
                colSpan={data?.table_columns?.length + 1 || 12}
            ...
            tableData.data.map((item: any) => (
              <TableRow key={item.id} className="even:bg-gray-50">
                ...
                <TableCell>{item.unit_kerja_pegawai}</TableCell>
                <TableCell className="text-center">
                  {item.jabatan_fungsional_pegawai}
                </TableCell>
                <TableCell>{item.nama_organisasi}</TableCell>
                <TableCell>{item.jabatan_dalam_organisasi}</TableCell>
                <TableCell className="text-center capitalize">
                  {item.jenis_organisasi}
                </TableCell>
                <TableCell className="text-center">{item.periode}</TableCell>
                <TableCell>{item.tempat_organisasi}</TableCell>
                <TableCell className="text-center capitalize">
                  <Button
                    size="sm"
                    variant={item.status_pengajuan_info?.color || "secondary"}
                    className="cursor-default"
                  >
                    {item.status_pengajuan_info?.label}
                  </Button>
                </TableCell>
                <TableCell>
