...
const KemampuanBahasa = () => {
  ...
  const bahasaFilter = searchParam.get("bahasa") || "";
  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    queryFn: async () => {
      ...
      const response = await adminServices.getKemampuanBahasa(params);
  ...
  const filterOptions = useMemo(() => {
    ...
    // Fungsi ini menangani dua format: {id, nama} dan {value, label}
    ...
    return {
      ...
      bahasa: mapToOptions(filters.bahasa_options, "value", "label"),
  ...
  // const { mutate: draftMutation } = useMutation({ ... }); // Bisa ditambahkan jika perlu
  ...
  if (isLoading) {
    return (
      <div className="mt-10 mb-20">
        <Title title="Validasi Kemampuan Bahasa" subTitle="Kompetensi" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <SelectFilter
                ...
                options={filterOptions.bahasa || []}
                value={bahasaFilter}
                onValueChange={(v) => handleUrlChange("bahasa", v)}
              />
            </div>
            <div className="flex flex-col gap-2">
      ...
      <Table className="mt-5 text-xs lg:text-sm">
        ...
        <TableBody>
          {tableData.data?.length === 0 ? (
            <TableRow>
              <TableCell
                colSpan={data?.table_columns?.length + 1 || 13}
            ...
            tableData.data.map((item: any) => (
              <TableRow key={item.id} className="even:bg-gray-50">
                ...
                <TableCell>{item.nama_bahasa}</TableCell>
