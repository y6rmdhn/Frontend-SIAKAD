...
const DetailJabatanFungsional = () => {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["detail-jabatan-fungsional-admin", params.id],
    queryFn: async () => {
      if (!params.id) {
        throw new Error("ID Jabatan Fungsional tidak ditemukan di URL");
      }
      const response = await adminServices.getJabatanFungsionalDetailAdmin(
        params.id
      );
    ...
    return (
      <div className="mt-10 mb-20">
        <Title
          ...
          subTitle="Detail Jabatan Fungsional"
        />
        <CustomCard>
      ...
      <div className="mt-10 mb-20 text-center">
        ...
        <CustomCard>
          <p className="p-10 text-red-500">
            Gagal memuat detail data jabatan fungsional.
          </p>
        </CustomCard>
      </div>
    );
          ...
          <div className="flex flex-col gap-4">
            <div className="flex justify-end">
              <Link
                ...
                to="/admin/validasi-data/kepegawaian/jabatan-fungsional"
              >
              ...
              <div className="space-y-2">
                <div className="flex flex-col sm:flex-row gap-2 justify-between border-b p-2">
                  ...
                  <Label className="text-xs sm:text-sm text-right">
                    {data?.data?.nama_jabatan_fungsional || "-"}
                  </Label>
                </div>
                <div className="flex flex-col sm:flex-row gap-2 justify-between border-b p-2">
                  <Label className="font-semibold text-[#2572BE] text-xs sm:text-sm shrink-0 w-38">
                  ...
                  <Label className="text-xs sm:text-sm text-right">
                    {data?.data?.tmt_jabatan_formatted || "-"}
                  </Label>
                </div>
                <div className="flex flex-col sm:flex-row gap-2 justify-between border-b p-2">
                  <Label className="font-semibold text-[#2572BE] text-xs sm:text-sm shrink-0 w-38">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-col sm:flex-row gap-2 justify-between items-start border-b p-2">
                  ...
                  <div className="text-xs sm:text-sm text-right">
                    {data?.data?.file_sk_jabatan_link ? (
                      <a
                        href={data.data.file_sk_jabatan_link}
                        target="_blank"
