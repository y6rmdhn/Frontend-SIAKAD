...
const Pangkat = () => {
  ...
  const pangkatFilter = searchParam.get("pangkat") || "";
  ...
  const { data, isLoading, isError, error } = useQuery({
    ...
    queryFn: async () => {
      const params = {
        ...
        pangkat: pangkatFilter,
        status_pengajuan: statusFilter,
      };
      const response = await adminServices.getPangkatValidasiData(params);
  ...
  const filterOptions = useMemo(() => {
    ...
    return {
      ...
      pangkat: mapToOptions(options.pangkat),
  ...
  const handleSuccess = (action: ActionType) => {
    ...
    queryClient.invalidateQueries({ queryKey: ["pangkat-validasi-data"] });
  };
  ...
  const { mutate: rejectMutation } = useMutation({
    mutationFn: (payload: { ids: number[]; keterangan: string }) =>
      patchDataServices.rejectDataPangkat(payload),
  ...
  const { mutate: approveMutation } = useMutation({
    mutationFn: (payload: { ids: number[] }) =>
      patchDataServices.approveDataPangkat(payload),
  ...
  const { mutate: draftMutation } = useMutation({
    mutationFn: (payload: { ids: number[] }) =>
      patchDataServices.draftDataPangkat(payload),
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div className="flex flex-col gap-2">
              ...
              <SelectFilter
                ...
                options={filterOptions.pangkat || []}
                value={pangkatFilter}
                onValueChange={(v) => handleUrlChange("pangkat", v)}
              />
            </div>
            <div className="flex flex-col gap-2">
          ...
          {tableData.data?.length === 0 ? (
            ...
            tableData.data.map((item: any) => (
              <TableRow key={item.id} className="even:bg-gray-50">
                ...
                <TableCell>{item.nama_pangkat}</TableCell>
                <TableCell className="text-center">
