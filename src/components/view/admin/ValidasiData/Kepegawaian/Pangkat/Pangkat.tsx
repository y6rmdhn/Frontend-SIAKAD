...
const rejectSchema = z.object({
  keterangan_penolakan: z.string().min(10, "Keterangan penolakan minimal 10 karakter."),
});
...
interface PangkatItem {
  ...
  master_pangkat?: { nama: string };
  tmt_pangkat: string | null;
  jenis_sk: string | null;
  no_sk: string | null;
  status: string;
...
const Pangkat = () => {
  ...
  const { data: rawData, isError, isLoading, error } = useQuery<PaginatedData>({
    queryKey: ["pangkat-validasi", currentPage, debouncedInput, statusFilter, startDate, endDate, unitKerjaId],
    queryFn: async () => {
      ...
      const response = await adminServices.getPangkatValidasiData(params as any);
  ...
  const handleSuccess = (action: string) => {
    ...
    queryClient.invalidateQueries({ queryKey: ["pangkat-validasi"] });
  };
  ...
  const { mutate: rejectMutation, isPending: isRejecting } = useMutation({
    mutationFn: (payload: { ids: string[]; keterangan_penolakan: string }) => patchDataServices.rejectDataPangkat(payload as any),
    onSuccess: () => handleSuccess("menolak"),
    onError: handleError,
  });
  const { mutate: approveMutation, isPending: isApproving } = useMutation({
    mutationFn: (payload: { ids: string[] }) => patchDataServices.approveDataPangkat(payload),
  ...
  const handleSubmitAction = (values: RejectSchema) => {
    if (pendingAction === "reject") rejectMutation({ ids: selectedItem, keterangan_penolakan: values.keterangan_penolakan });
  };

  if (isLoading && !rawData) {
    return (
      <div className="mt-10 mb-20 space-y-6">
        <Title title="Validasi Data Pangkat" />
    ...
    <div className="mt-10 mb-20">
      ...
      <div className="flex flex-col md:flex-row md:justify-between mt-10 gap-4">
        ...
        <div className="flex flex-col md:flex-row gap-2 items-start md:items-center">
          ...
          <Link to="/admin/validasi-data/kepegawaian/pangkat/tambah-pangkat">
        ...
        <Table className="text-xs lg:text-sm">
          <TableHeader>
            <TableRow className="bg-gray-100 hover:bg-gray-100">
              ...
              <TableHead className="text-center">TMT Pangkat</TableHead>
            ...
            {items.length === 0 ? (
              ...
              items.map((item) => (
                <TableRow key={item.id} className="even:bg-gray-50">
                  ...
                  <TableCell className="text-center">
                    <Link to={`/admin/validasi-data/kepegawaian/pangkat/detail-pangkat/${item.id}`}>
          ...
          {pendingAction === "reject" && (
            <Form {...form}>
              <form id="action-form" onSubmit={form.handleSubmit(handleSubmitAction)}>
                <div className="py-4">
                  <FormFieldInput form={form} name="keterangan_penolakan" label="Keterangan Penolakan" placeholder="Masukkan alasan penolakan..." required />
                </div>
              </form>
            </Form>
          )}
