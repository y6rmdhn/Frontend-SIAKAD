...
// 1. Mengganti nama komponen agar sesuai dengan nama file
const DetailJabatanAkademik = () => {
  const { id } = useParams<{ id: string }>();

  const { data, isLoading, isError } = useQuery({
    // 2. Mengganti queryKey agar unik
    queryKey: ["jabatan-akademik-detail", id],
    queryFn: async () => {
      if (!id) throw new Error("ID tidak ditemukan");
      // 3. Memanggil service yang sesuai
      const response = await adminServices.getJabatanAkademikDetail(id);
  ...
  if (isLoading) {
    return (
      <div className="mt-10 mb-20">
        <Title
          title="Data Jabatan Akademik"
          subTitle="Detail Jabatan Akademik"
        />
        <CustomCard
          actions={
    ...
    return (
      <div className="mt-10 mb-20 text-center">
        <Title
          ...
          subTitle="Detail Jabatan Akademik"
        />
        <p className="text-red-500">Gagal memuat data jabatan akademik.</p>
      </div>
    );
  }
          ...
          <div className="flex flex-col gap-4">
            <div className="flex justify-end">
              <Link
                ...
                to="/admin/validasi-data/kepegawaian/jabatan-akademik"
              >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-4 border border-gray-200 rounded-lg p-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-col sm:flex-row gap-2 justify-between border-b p-2">
                  ...
                  <Label className="text-sm font-medium text-right">
                    {data?.data.nama_jabatan_akademik || "-"}
                  </Label>
                </div>
                <div className="flex flex-col sm:flex-row gap-2 justify-between border-b p-2">
                  <Label className="font-semibold text-sm text-[#2572BE] shrink-0 w-48">
                  ...
                  <Label className="text-sm font-medium text-right">
                    {formatDate(data?.data.tmt_jabatan)}
                  </Label>
                </div>
                <div className="flex flex-col sm:flex-row gap-2 justify-between border-b p-2">
                  <Label className="font-semibold text-sm text-[#2572BE] shrink-0 w-48">
