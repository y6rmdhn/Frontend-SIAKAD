...
const JabatanAkademik = () => {
  ...
  const { data: rawData, isError, isLoading, error } = useQuery<PaginatedData>({
    queryKey: ["jabatan-akademik-validasi", currentPage, debouncedInput, statusFilter, startDate, endDate, unitKerjaId],
    queryFn: async () => {
      ...
      const response = await adminServices.getJabatanAkademikValidasiData(params as any);
  ...
  const { mutate: rejectMutation, isPending: isRejecting } = useMutation({
    mutationFn: (payload: { ids: string[]; keterangan: string }) => patchDataServices.rejectDataRiwayatJabatanAkademik(payload),
    onSuccess: () => handleSuccess("menolak"),
    onError: handleError,
  });
  const { mutate: approveMutation, isPending: isApproving } = useMutation({
    mutationFn: (payload: { ids: string[] }) => patchDataServices.approveDataRiwayatJabatanAkademik(payload),
    ...
    <div className="mt-10 mb-20">
      ...
      <div className="flex flex-col md:flex-row md:justify-between mt-10 gap-4">
        ...
        <div className="flex flex-col md:flex-row gap-2 items-start md:items-center">
          ...
          <Link to="/admin/validasi-data/kepegawaian/jabatan-akademik/tambah-jabatan-akademik">
        ...
        <Table className="text-xs lg:text-sm">
          <TableHeader>
            <TableRow className="bg-gray-100 hover:bg-gray-100">
              ...
              <TableHead className="text-center">TMT Jabatan</TableHead>
            ...
            {items.length === 0 ? (
              ...
              items.map((item) => (
                <TableRow key={item.id} className="even:bg-gray-50">
                  ...
                  <TableCell className="text-center">{formatDate(item.tmt_jabatan)}</TableCell>
                  ...
                  <TableCell className="text-center">
                    <Link to={`/admin/validasi-data/kepegawaian/jabatan-akademik/detail-jabatan-akademik/${item.id}`}>
