...
const Keluarga = () => {
  ...
  const handleSuccess = (action: string) => {
    ...
    queryClient.invalidateQueries({ queryKey: ["keluarga-validasi"] });
  };
  ...
  const { mutate: rejectMutation, isPending: isRejecting } = useMutation({
    mutationFn: (payload: { ids: string[]; keterangan: string }) => patchDataServices.tolakDataKeluarga(payload),
    onSuccess: () => handleSuccess("menolak"),
    onError: handleError,
  });
  const { mutate: approveMutation, isPending: isApproving } = useMutation({
    mutationFn: (payload: { ids: string[] }) => patchDataServices.approveDataKeluarga(payload),
  ...
  if (isLoading && !rawData) {
    return (
      <div className="mt-10 mb-20 space-y-6">
        <Title title="Validasi Data Keluarga" />
        ...
        <Table className="text-xs lg:text-sm">
          <TableHeader>
            <TableRow className="bg-gray-100 hover:bg-gray-100">
              ...
              <TableHead className="text-center">Hubungan</TableHead>
            ...
            {items.length === 0 ? (
              ...
              items.map((item) => (
                <TableRow key={item.id} className="even:bg-gray-50">
                  ...
                  <TableCell className="text-center">
                    {item.status === "disetujui" ? item.tgl_disetujui ?? "-" : item.status === "ditolak" ? item.tgl_ditolak ?? "-" : item.tgl_diajukan ?? "-"}
                  </TableCell>
                  <TableCell className="text-center">
