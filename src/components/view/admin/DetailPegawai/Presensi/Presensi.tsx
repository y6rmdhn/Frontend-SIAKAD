...
const Presensi = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: [
      "riwayat-presensi-detail-pegawai",
      params.id,
      searchParam.get("page"),
      searchParam.get("search"),
    ],
    queryFn: async () => {
      const response = await adminServices.getRiwayatPresensiDetailPegawai(
  ...
  useEffect(() => {
    ...
    if (debouncedInput.length > 0) {
      // Adjusted for year/month filter if needed
        ...
        <div className="flex flex-col gap-4 w-full">
          ...
          <CustomCard
            actions={
              <div className="flex gap-4 flex-col md:flex-row">
                <Label className="md:pr-30 text-[#FDA31A]">Tahun</Label>
                <SelectFilter
                  classname="w-full md:w-80"
                  options={unitKerjaOptions} // Should be a list of years
          ...
          <div className="gap-5 flex flex-col md:flex-row">
            <SelectFilter
              ...
              options={unitKerjaOptions} // Should be a list of months
                ...
                <TableRow key={item.id}>
                  ...
                  <TableCell className="h-full">
                    <div className="flex justify-center items-center w-full h-full">
                      <Link to={`/admin/presensi/detail/${item.id}`}>
                        <Button
                          size="icon"
                          variant="ghost"
