...
const PendidikanFormal = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: [
      "pendidikan-formal-detail-pegawai",
      params.id,
      searchParam.get("page"),
      searchParam.get("search"),
    ],
    queryFn: async () => {
      const response = await adminServices.getPendidikanFormalDetailPegawai(
  ...
  return (
    <DetailPegawaiLayout
      ...
      subTitile="Daftar Pendidikan Formal"
    >
      {/*Sidebar*/}
        ...
        <Table className="mt-10 table-auto border-separate border-spacing-x-[1px]">
          <TableHeader>
            <TableRow className="bg-[#004680]">
              ...
              <TableHead className="text-center text-white">Gelar</TableHead>
            ...
            {isLoading ? (
              ...
              data.data.data.map((item: any) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell className="text-center">{item.jenjang}</TableCell>
                  <TableCell className="text-center">{item.gelar}</TableCell>
                  ...
                  <TableCell className="h-full">
                    <div className="flex justify-center items-center w-full h-full">
                      <Link
                        to={`/admin/kepegawaian/pendidikan-formal/detail/${item.id}`}
                      >
                        <Button
                          size="icon"
                          variant="ghost"
