...
const Biodata = () => {
  ...
  const { data, isLoading } = useQuery<PegawaiDetail>({
    ...
    queryFn: async () => {
      ...
      const response = await adminServices.getPegawaiDetailAdminPage(Number(params.id));
  ...
  const dataLainLainLeft = [
    data?.karpeg || "-",
    data?.file_karpeg ? (
        <a href={`http://103.158.196.106/storage/${data.file_karpeg}`} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">
          Lihat File Karpeg
        </a>
    ) : ("-"),
    data?.npwp || "-",
    data?.file_npwp ? (
        <a href={`http://103.158.196.106/storage/${data.file_npwp}`} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">
          Lihat File NPWP
        </a>
    ) : ("-"),
  ];
