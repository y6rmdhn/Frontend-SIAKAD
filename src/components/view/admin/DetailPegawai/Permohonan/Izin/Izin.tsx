...
const Izin = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await adminServices.getIzinDetailPegawai(
  ...
  return (
    <DetailPegawaiLayout title="Izin" subTitile="Daftar Permohonan Izin">
      {/*Sidebar*/}
        ...
        <Table className="mt-10 table-auto border-separate border-spacing-x-[1px]">
          <TableHeader>
            <TableRow className="bg-[#004680]">
              ...
              <TableHead className="text-center text-white">Lama</TableHead>
              <TableHead className="text-center text-white">
                Status Pengajuan
              </TableHead>
            ...
            {isLoading ? (
              ...
              data.data.data.map((item: any) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell className="text-center">{item.lama}</TableCell>
                  <TableCell className="text-center capitalize">
                    {item.status_pengajuan}
                  </TableCell>
                  <TableCell className="h-full">
                    <div className="flex justify-center items-center w-full h-full">
                      <Link to={`/admin/kepegawaian/izin/detail/${item.id}`}>
                        <Button
                          size="icon"
                          variant="ghost"
