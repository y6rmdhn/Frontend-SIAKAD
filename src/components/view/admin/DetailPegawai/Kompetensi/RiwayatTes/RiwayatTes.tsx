...
const RiwayatTes = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await adminServices.getTesDetailPegawai(
        ...
        <Table className="mt-10 table-auto border-separate border-spacing-x-[1px]">
          <TableHeader>
            <TableRow className="bg-[#004680]">
              ...
              <TableHead className="text-center text-white">Nama Tes</TableHead>
            ...
            {isLoading ? (
              ...
              data.data.data.map((item: any) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell className="text-center">{item.skor_tes}</TableCell>
