...
const Organisasi = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: [
      "organisasi-detail-pegawai",
      params.id,
      searchParam.get("page"),
      searchParam.get("search"),
    ],
    queryFn: async () => {
      const response = await adminServices.getOrganisasiDetailPegawai(
  ...
  return (
    <DetailPegawaiLayout title="Organisasi" subTitile="Daftar Organisasi">
      {/*Sidebar*/}
        ...
        <Table className="mt-10 table-auto border-separate border-spacing-x-[1px]">
          <TableHeader>
            <TableRow className="bg-[#004680]">
              ...
              <TableHead className="text-center text-white">Jabatan</TableHead>
              <TableHead className="text-center text-white">Waktu</TableHead>
              <TableHead className="text-center text-white">Lingkup</TableHead>
              <TableHead className="text-center text-white">
                Status Pengajuan
              </TableHead>
                ...
                <TableRow key={item.id}>
                  ...
                  <TableCell className="h-full">
                    <div className="flex justify-center items-center w-full h-full">
                      <Link
                        to={`/admin/kepegawaian/organisasi/detail/${item.id}`}
                      >
                        <Button
                          size="icon"
                          variant="ghost"
