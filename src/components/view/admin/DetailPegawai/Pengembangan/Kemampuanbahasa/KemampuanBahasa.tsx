...
const KemampuanBahasa = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: [
      "kemampuan-bahasa-detail-pegawai",
      params.id,
      searchParam.get("page"),
      searchParam.get("search"),
    ],
    queryFn: async () => {
      const response = await adminServices.getKemampuanBahasaDetailPegawai(
  ...
  return (
    <DetailPegawaiLayout
      ...
      subTitile="Daftar Kemampuan Bahasa"
    >
      {/*Sidebar*/}
        ...
        <Table className="mt-10 table-auto border-separate border-spacing-x-[1px]">
          <TableHeader>
            <TableRow className="bg-[#004680]">
              ...
              <TableHead className="text-center text-white">Tahun</TableHead>
              <TableHead className="text-center text-white">Bahasa</TableHead>
            ...
            {isLoading ? (
              ...
              data.data.data.map((item: any) => (
                <TableRow key={item.id}>
                  ...
                  <TableCell className="text-center">{item.bahasa}</TableCell>
                  <TableCell className="text-center">
                    {item.nilai_mendengar}
                  </TableCell>
                  <TableCell className="text-center">
                    {item.nilai_bicara}
                  </TableCell>
                  ...
                  <TableCell className="h-full">
                    <div className="flex justify-center items-center w-full h-full">
                      <Link
                        to={`/admin/kepegawaian/kemampuan-bahasa/detail/${item.id}`}
                      >
                        <Button
                          size="icon"
                          variant="ghost"
