...
const StatusKeaktifan = () => {
    ...
    const {mutate: putStatusAktif} = useMutation({
        mutationFn: (data: statusKeaktifanFormvalue) => putReferensiServices.statusAktif(data.id!, data),
        onSuccess: () => {
            form.reset();
    ...
    const {mutate: deleteStatusAktif} = useMutation({
        mutationFn: (id: number) => deleteReferensiServices.deteleStatusAktif(id),
        onSuccess: () => {
