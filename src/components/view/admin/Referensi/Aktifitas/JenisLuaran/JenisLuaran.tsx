...
const JenisLuaran = () => {
  ...
  const { mutate: postJenisLuaran } = useMutation({
    mutationFn: (data: IJenisLuaran) => potsReferensiServices.jenisLuaran(data),
    onSuccess: () => {
      form.reset();
  ...
  const { mutate: putJenisLuaran } = useMutation({
    mutationFn: (data: jenisLuaranFormvalue) =>
      putReferensiServices.jenisLuaran(data.id!, data),
    onSuccess: () => {
      form.reset();
  ...
  const { mutate: deleteStatusAktif } = useMutation({
    mutationFn: (id: string) => deleteReferensiServices.deteleJenisLuaran(id),
    onSuccess: () => {
